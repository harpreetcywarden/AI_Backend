...
def process_pdf(file_path):
    ...
    try:
        ...
        if pdf_text:
            ...
            logger.debug(f"📄 Extracted {len(pdf_text)} text pages from PDF: {os.path.basename(file_path)}")
        ...
        try:
            ...
            logger.debug(f"🖼️ Extracted {len(images)} images from PDF for OCR: {os.path.basename(file_path)}")
            ocr_text = []
            for i, image in enumerate(images):
                ...
                try:
                    ...
                    logger.warning(f"⚠️ OCR timed out for an image in {os.path.basename(file_path)}: {timeout_error}")
                except pytesseract.TesseractError as ocr_err:
                     logger.warning(f"⚠️ OCR failed for an image in {os.path.basename(file_path)}: {ocr_err}")
                except Exception as img_err:
                     logger.warning(f"⚠️ Error processing an image for OCR in {os.path.basename(file_path)}: {img_err}")
                finally:
            ...
            if ocr_text:
                ...
                logger.debug(f"👁️ Added {len(ocr_text)} chunks from OCR for PDF: {os.path.basename(file_path)}")

        except Exception as pdf_img_err:
            # Handle pdf2image errors (e.g., Poppler not found, file issues) gracefully
            logger.warning(f"⚠️ Could not extract/process images from PDF {os.path.basename(file_path)}: {pdf_img_err}")

        return combined_text if combined_text else []
    except Exception as e:
        logger.error(f"❌ Error processing PDF {os.path.basename(file_path)}: {str(e)}", exc_info=True)
        return []
...
def file_to_opensearch(
    ...
    for file_path in file_paths:
        ...
        # logger.info(f"⚙️ Processing file: {original_filename} (Entry: {entry_uuid}, Type: {file_type})") # Log removed type
    ...
    return (response, 
            { "status": "error" if errors_occurred else "success", 
             "message": f"Processed {files_processed_count} files. Indexed {total_chunks_indexed_session} chunks." + (" Errors occurred." if errors_occurred else ""), 
             "indexed_chunks": total_chunks_indexed_session, "index_name": index_name })
...
async def process_file_endpoint(
    ...
    def background_process_and_index(temp_dir: str, local_file_path: str): # <<< Updated: Removed type param
        ...
        processing_result = {}
        try:
            # logger.info(f"🚀 Starting background processing: entry={entry_uuid}, user={user_id}, file={original_filename}, type={type_for_task}") # Original log
...
@app.post("/vectors/search", response_model=SearchResponse) # Use the response model
async def search_documents_endpoint(
    payload: SearchRequestPayload,
    # request: Request, # Add if needed for logging or other request details
    is_key_valid: bool = Depends(verify_api_key) # Reuse API key verification
):
    """
    ...
    query = payload.query
    top_k = payload.top_k
    # min_score = payload.min_score # Uncomment if using min_score
    ...
    # --- Determine User-Specific Index Name & Check Existence ---
    try:
        import re as re
        safe_user_id_part = re.sub(r'[^a-z0-9_-]', '_', str(user_id).lower())
        if not safe_user_id_part:
            raise ValueError("Invalid user_id for index name generation.") # Should be caught by below handler
        ...
        if not opensearch.indices.exists(index=index_name):
            ...
            # Return empty results for a non-existent index, or 404
            # raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"No documents found for user {user_id}.")
            return SearchResponse(results=[], index_queried=index_name) # Return empty list
    ...
    search_body = {
        "size": top_k, # Limit the number of final results returned
        "_source": ["chunk_id", "content", "original_filename", "s3_key", "entry_uuid"], # Specify fields to retrieve
        "query": {
            "knn": {
                "embedding": { # Name of your vector field
                    "vector": query_vector,
                    "k": top_k # How many nearest neighbors for the KNN algorithm to find initially
                    # Optional filter can be added here if needed
                    # "filter": { "term": { "metadata_field": "value" }}
                }
            }
        }
        # Optional: Add min_score threshold
        # "min_score": min_score if min_score is not None else 0.0 # Only return results above a certain score
    }
    ...
    try:
        ...
        logger.info(f"Search completed. Found {len(response['hits']['hits'])} potential hits.")
         ...
         # Look for specific errors in re.info['error']['root_cause']
    ...
    for hit in response['hits']['hits']:
        ...
        score = hit.get('_score', 0.0) # Default score to 0.0 if missing

        # Add only if source has the required fields
